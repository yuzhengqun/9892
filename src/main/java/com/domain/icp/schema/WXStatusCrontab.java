...
@Component
public class WXStatusCrontab {
	private static Logger logger = LoggerFactory
			.getLogger(WXStatusCrontab.class);
	...
	// @Scheduled(cron = "0/2 * * * * *")
	public void exe2() {
		logger.info("check wx status begin");
		// 尚未查询微信状态的列表
		this.exeStatus(2);
		logger.info("check wx status end.");
	...
	private void getWxStatus(DomainInfo domainInfo) throws Exception {
		...
		String line = domainInfo.getWxStatusUrl();
		...
		if ("未知".equals(response)) {
			Thread.sleep(1000 * 3);
			return;
		}
		Document doc = Jsoup.parse(response);
		String content = doc.selectFirst("font").text();
		if ("未拦截".equals(content)) {
			domainInfo.setWxStatus(1);
		} else if ("已拦截".equals(content)) {
			domainInfo.setWxStatus(0);
		}
	}
}
